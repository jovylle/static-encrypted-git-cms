...
import { filterPublicList, loadPublicJson } from './composables/usePublicData.js';
...
async function load(url) {
  ...
  try {
    let json = await loadPublicJson(url);
...
watch(active, (ep) => load(ep.url));
onMounted(() => {
  if (!previewData.value && !previewError.value) load(active.value.url);
});
onServerPrefetch(async () => {
  await load(active.value.url);
});
